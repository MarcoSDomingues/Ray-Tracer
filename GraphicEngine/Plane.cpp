...
bool Plane::checkIntersection(const Ray &ray, Vector3 &hitpoint, float &tmin, float &distance, Vector3 &normal, Material &m) {
	...
	distance = (a - origin).length();
	float distAux = -(a - origin).dot(normal);
